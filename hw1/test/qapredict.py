...
def main():
    args = parse_args()
    send_example_telemetry("predict_qa", args)
    ...
    def mappingResults(examples):
        examples["answer"] = prediction.predictions[identifyid.index(examples["id"])]["prediction_text"]
        all_keys = list(examples)
        for key in all_keys:
            if key == "id" or key == "answer":
                continue
            examples.pop(key)
        return examples
    raw_datasets = raw_datasets.map(mappingResults)
    ...
    if args.output_dir is not None:
        ...
        result.to_json(os.path.join(args.output_dir,"qaresult_test.json"), **kwargs)
