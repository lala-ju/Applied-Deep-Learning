...
def main():
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        all_loss.append(f"train loss at epoch {epoch}: {total_loss / len(train_dataloader)}")
        all_match.append(f"train exact match at epoch {epoch}: {eval_metric}")
        logger.info(f"train loss at epoch {epoch}: {total_loss / len(train_dataloader)}")
        logger.info(f"train exact match at epoch {epoch}: {eval_metric}")
    ...
    if args.output_dir is not None:
        ...
        with open(os.path.join(args.output_dir, 'all_loss.txt'), 'w') as f:
            for line in all_loss:
                f.write(f"{line}\n")
        with open(os.path.join(args.output_dir,'all_match.txt'), 'w') as f:
